import type { CorporateRecord, IdentityInput, IdentityScore, IdentitySignal } from "./types.ts";
import { extractDomain, normalizeAddress, normalizeCompanyName } from "./normalize.ts";
...
export function scoreIdentityMatch(input: IdentityInput, candidate: CorporateRecord): IdentityScore {
  ...
  const nameMatched =
    !!inputName && (inputName === candidateName || inputName === candidateNameEn || candidateName.includes(inputName));
  signals.push({
  ...
  const numberMatched =
    ...
    isCorporateNumberFormat(input.corporateNumber) &&
    input.corporateNumber === candidate.corporateNumber;
  signals.push({
  ...
  const candidateAddress = normalizeAddress(
    candidate.address ?? [candidate.prefectureName, candidate.cityName, candidate.streetNumber].filter(Boolean).join(""),
  );
...
export function rankCandidates(input: IdentityInput, candidates: CorporateRecord[]): Array<CorporateRecord & IdentityScore> {
  return candidates
    .map((candidate) => ({ ...candidate, ...scoreIdentityMatch(input, candidate) }))
